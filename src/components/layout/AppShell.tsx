...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 border-b border-white/5 bg-[#0b1020]/75 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
          <Link href="/" className="group flex items-center gap-2">
            <div className="grid h-9 w-9 place-items-center rounded-xl bg-card/70 ring-1 ring-white/10">
            ...
            {nav.slice(0, 4).map((n) => {
              ...
              return (
                <Link
                  ...
                  href={n.href}
                  className={cn(
                    "relative rounded-xl px-3 py-2 text-sm text-white/70 transition hover:text-white",
                    active && "text-white",
                  )}
                >
                  ...
                  {active ? (
                    <motion.div
                      layoutId="nav"
                      className="absolute inset-0 -z-10 rounded-xl bg-card/60 ring-1 ring-accent/30"
                    />
                  ) : null}
                </Link>
              );
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="hidden rounded-xl bg-card/50 px-3 py-2 text-sm ring-1 ring-white/10 sm:block">
              <span className="text-white/60">Баланс</span>{" "}
              <span className="font-semibold text-white">{state?.balance ?? 0} ₽</span>
            </div>
            <button
              onClick={() => refill()}
              className="rounded-xl bg-accent px-3 py-2 text-sm font-semibold text-black transition hover:brightness-110"
            >
              Обновить до 1000 ₽
            </button>
          </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/5 bg-[#0b1020]/80 backdrop-blur md:hidden">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active && "bg-card/60 text-white ring-1 ring-accent/30",
                )}
              >
                <Icon className="h-4 w-4" />
                {n.label}
