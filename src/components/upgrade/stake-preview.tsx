...
export function StakePreview() {
  ...
  const stake =
    !bet
      ? null
      : bet.type === "balance"
        ? { title: "Баланс", subtitle: `${fmtMoney(bet.amount)} ₽`, image: null }
        : (() => {
            const skinId = bet.skinInstanceId.split("::")[1];
            const skin = SKIN_BY_ID.get(skinId);
            if (!skin) return { title: "Скин", subtitle: "Неизвестно", image: null };
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent>
        <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl bg-black/25 ring-soft">
          ...
          />
          {stake?.image ? (
            <Image src={stake.image} alt={stake.title} fill className="object-contain p-4 opacity-95" />
          ) : (
            <div className="absolute inset-0 grid place-items-center">
              <div className="text-sm text-white/45">{stake ? "Ставка выбрана" : "Скин не выбран"}</div>
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/10 to-transparent" />
          <div className="absolute bottom-3 left-3 right-3">
            <div className="text-sm font-semibold text-white/90">{stake ? stake.title : "—"}</div>
            <div className="mt-0.5 text-xs text-white/55">{stake ? <span className="break-words">{stake.subtitle}</span> : null}</div>
          </div>
        </div>
      </CardContent>
    </Card>
